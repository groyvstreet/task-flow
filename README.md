...
В качестве сервера используется **Firebase Realtime Database**.

Дополнительная настройка мокового сервера (json-server и т.п.) **не нужна** — приложение уже настроено на Firebase и синхронизируется с ним автоматически.
